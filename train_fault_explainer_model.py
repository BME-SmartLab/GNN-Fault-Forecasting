...
log_path = f'~/GNN-Fault-Forecasting/explainability_runs/{data_folder}/hidden_size_{hidden_size}_lr_{lr}_weight_decay_{weight_decay}_epochs_{epochs}_batch_size_{batch_size}'
...
if train_explainer:
    ...
    for epoch in range(epochs):
        train_loss, train_sparsity, train_pos_loss, train_neg_loss, train_pos_mae, train_neg_mae = explainer.train_batch(train_loader)
        val_loss, val_sparsity, val_pos_loss, val_neg_loss, val_pos_mae, val_neg_mae = explainer.test_batch(val_loader)
        ...
        print(f'Epoch: {epoch}, Train loss: {train_loss:.4f}, Train sparsity: {train_sparsity:.4f}, Train pos loss: {train_pos_loss:.4f}, Train neg loss: {train_neg_loss:.4f}, Train pos mae: {train_pos_mae:.4f}, Train neg mae: {train_neg_mae:.4f}')
        print(f'Epoch: {epoch}, Val loss: {val_loss:.4f}, Val sparsity: {val_sparsity:.4f}, Val pos loss: {val_pos_loss:.4f}, Val neg loss: {val_neg_loss:.4f}, Val pos mae: {val_pos_mae:.4f}, Val neg mae: {val_neg_mae:.4f}')
...
val_loss, val_sparsity, val_pos_loss, val_neg_loss, val_pos_mae, val_neg_mae = explainer.test_batch(val_loader)
test_loss, test_sparsity, test_pos_loss, test_neg_loss, test_pos_mae, test_neg_mae = explainer.test_batch(test_loader)
print(f'Val loss: {val_loss:.4f}, Val sparsity: {val_sparsity:.4f}, Val pos loss: {val_pos_loss:.4f}, Val neg loss: {val_neg_loss:.4f}, Val pos mae: {val_pos_mae:.4f}, Val neg mae: {val_neg_mae:.4f}')
print(f'Test loss: {test_loss:.4f}, Test sparsity: {test_sparsity:.4f}, Test pos loss: {test_pos_loss:.4f}, Test neg loss: {test_neg_loss:.4f}, Test pos mae: {test_pos_mae:.4f}, Test neg mae: {test_neg_mae:.4f}')
