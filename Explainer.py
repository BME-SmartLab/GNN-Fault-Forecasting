    ...
    def train_batch(self, loader):
        ...
        for data in loader:
            ...
            with torch.no_grad():
                ...
                pos_acc = (critic_pos_out.argmax(dim=1) == data.y).sum() / data.y.size(0)
                critic_pos_accs.append(pos_acc.item())
                critic_neg_out = self.critic(data, 1.0 - mask)
                neg_acc = (critic_neg_out.argmax(dim=1) == data.y).sum() / data.y.size(0)
                critic_neg_accs.append(neg_acc.item())
                baseline_out = self.baseline(data)
                baseline_acc = (baseline_out.argmax(dim=1) == data.y).sum() / data.y.size(0)
